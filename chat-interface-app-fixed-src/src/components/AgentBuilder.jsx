...
import {
  ChevronDown, ChevronUp, Bot, Plus, Trash2
} from 'lucide-react';

// --- Reusable UI Components (scoped to this builder) ---
...
const TaskManager = ({ tasks, onTasksChange }) => {
    const addTask = () => {
        const newTask = { name: '', description: '', endpoint: '', params_schema: { type: 'object', properties: {}, required: [] } };
...
// --- Main Agent Team Builder Component ---

const AgentBuilder = ({ agents, onUpdate }) => {
    const addAssistant = () => {
        onUpdate([...agents, { name: '', system_message: '', tasks: [] }]);
    };
                ...
                {agents.map((agent, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg bg-white shadow-sm p-4">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-semibold text-lg text-gray-800">{agent.name || `Agent ${index + 1}`}</h3>
                            {agents.length > 1 && (
